...
func (r *Runner) Run(ctx context.Context) error {
	if r.cfg.Interval <= 0 { return errors.New("patrol interval must be greater than zero") }
	if r.cfg.Timeout <= 0 { return errors.New("patrol timeout must be greater than zero") }
	if r.cfg.RunOnBoot {
